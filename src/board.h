...
struct Board {
  ...
  void setFen(const std::string &fen) {
    ...
    for (int i = 0; fen[i] != ' '; ++i) {
      if (fen[i] == '/') {
        ...
        insertPeca(BitBoard::make_place(linha, coluna), type, owner);
        ++coluna;
      }
    }
    for (int i = 0;; ++i) {
  ...
  MoveInfo move(Move m) {
    ...
    if (type == PAWN and (not mi.capturou) and m.dc() != m.oc()) {
      int p = BitBoard::make_place(m.ol(), m.dc());
      mi.p = b[p];
      mi.fez_passan = true;
      erasePeca(p);
    } else {
  ...
  void volta(Move m, const MoveInfo &mi) {
    ...
    /* desfaz passan */
    if (mi.fez_passan) {
      insertPeca(BitBoard::make_place(m.ol(), m.dc()), mi.p.type, mi.p.owner);
    }
  ...
  std::string getFen() {
    ...
    for (int l = 7; l >= 0; --l) {
      for (int c = 0; c < 8; ++c) {
        fen += tabela[b[BitBoard::make_place(l, c)].owner]
                     [b[BitBoard::make_place(l, c)].type];
      }
      fen += '/';
    }
  ...
  void print(FILE *f) {
    ...
    for (int l = 7;; l--) {
      for (int c = 0; c < 8; c++) {
        linha += '|';
        if (b[BitBoard::make_place(l, c)].type == CLEAR) {
          linha += "  ";
        } else {
          linha += tabela[(int)b[BitBoard::make_place(l, c)].type];
          linha += tabela2[b[BitBoard::make_place(l, c)].owner];
        }
      }
      linha += '|';
      fprintf(f, "%s\n", linha.c_str());
  ...
  bool checkBoard() {
    for (int l = 0; l < 8; ++l) {
      for (int c = 0; c < 8; ++c) {
        int place = BitBoard::make_place(l, c);
  ...
  bool checkPawnMove(const Move &m, int owner) const {
    ...
    } else if ((m.oc() > 0 and m.ol() + dir == m.dl() and
                m.oc() - 1 == m.dc()) or
               ((m.oc() < 7 and m.ol() + dir == m.dl() and
                 m.oc() + 1 == m.dc()))) {
  ...
  NOINLINE int countQueenMovesg(int color, int place, Move *list) {
    ...
    BitBoard dest = BitBoard::get_queen_moves(
                        place, block | this->bb_blockers[OPPONENT(color)]) &
                    ~(block);
    return dest.pop_count();
  }
    ...
    for (int t = 0; t < 2; ++t) {
      ...
      for (int i = 0; i < size_table[t][PAWN]; ++i) {
        ...
        if ((BitBoard::get_passed_pawn_mask(t, p) & bbPeca[op][PAWN]) == 0) {
          pawn_score += passed_pawn_score[t][BitBoard::line_of_place(p)];
        }
    ...
    for (int t = 0; t < 2; ++t) {
      for (int i = 0; i < size_table[t][KNIGHT]; ++i) {
        mob_score += mobility_score[t][KNIGHT] *
                     countKnightMovesg(t, peca_table[t][KNIGHT][i].place);
      }
      for (int i = 0; i < size_table[t][BISHOP]; ++i) {
        mob_score += mobility_score[t][BISHOP] *
                     countBishopMovesg(t, peca_table[t][BISHOP][i].place);
      }
      for (int i = 0; i < size_table[t][ROOK]; ++i) {
        mob_score += mobility_score[t][ROOK] *
                     countRookMovesg(t, peca_table[t][ROOK][i].place);
      }
    }

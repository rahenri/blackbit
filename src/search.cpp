...
namespace {
struct init_search {
  init_search() { memset(move_history, 0, sizeof(move_history)); }
} _init_search;
} // namespace

void sort_moves(const Board &board, Move *moves, int n) {
  MoveScore ms[512];
  for (int i = 0; i < n; ++i) {
    ms[i].m = moves[i];
    ms[i].setScore(board);
...
int search(Board &board, int depth, int alfa = -INFINITE_SCORE,
           int beta = INFINITE_SCORE, int flags = SEARCH_ROOT) {
  int n, t, i;
  MoveInfo mi;
  hash_slot *slot;
  Move list[512];
  ...
  if (not is_quiescent) {
    ...
    if (depth >= 3 and slot == 0) {
      search(board, depth - 2, alfa, beta, SEARCH_PV);
      slot = hash_find(board);
    }
  ...
  sort_moves(board, list + list_offset, n - list_offset);
  ...
  for (i = 0; i < n; i++) {
    ...
    if (i > 0 and not is_pv and depth > 1) {
      t = -search(board, depth - 1, -best_score - 1, -best_score, flags);
    }

    if (t > best_score) {
      t = -search(board, depth - 1, -beta, -std::max(alfa, best_score),
                  flags | SEARCH_PV);
    }
  ...
  if (not is_quiescent) {
    if (best_score <= alfa) {
      /* upper bound */
      hash_insert(board, depth, -(mate_score + depth - 1), best_score,
                  best_move);
